...
\n+        cDriver.m_cLocation.lines(yyleng); cDriver.m_cLocation.step();

"-"        return yy::parser::make_MINUS  (cDriver.m_cLocation);
"+"        return yy::parser::make_PLUS   (cDriver.m_cLocation);
"*"        return yy::parser::make_STAR   (cDriver.m_cLocation);
"/"        return yy::parser::make_SLASH  (cDriver.m_cLocation);
"("        return yy::parser::make_LPAREN (cDriver.m_cLocation);
")"        return yy::parser::make_RPAREN (cDriver.m_cLocation);
":="       return yy::parser::make_ASSIGN (cDriver.m_cLocation);

{int}      return make_NUMBER (yytext, cDriver.m_cLocation);
{id}       return yy::parser::make_IDENTIFIER (yytext, cDriver.m_cLocation);
.          {
             throw yy::parser::syntax_error
               (cDriver.m_cLocation, "invalid character: " + std::string(yytext));
           }
